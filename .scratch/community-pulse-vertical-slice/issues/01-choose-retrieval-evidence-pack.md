...
3. [Add the optional local vector upgrade](12-add-local-vector-retrieval.md) comes last, after the BM25-backed grounded-answer evaluation baseline in issue 11. Only after the BM25-backed flow works and its baseline is recorded does it add embeddings, local Postgres/`pgvector`, top-40 semantic candidates, and reciprocal-rank fusion. Any unavailable, stale, or partial vector state falls back to the working `bm25` mode rather than blocking the answer.

### Target hybrid upgrade (issue 12)
...
4. Run two date-filtered searches: the vector SQL query returns its top 40 by exact cosine distance, with `timestamp DESC, id ASC` as stable secondary ordering, and the dependency-free BM25 index returns its top 20 using `k1 = 1.2`, `b = 0.75`, with the same secondary ordering for equal scores. Both paths use the query after removing the date phrase. BM25 normalizes with Unicode NFKC, lowercase, punctuation-to-space, whitespace collapse, and the versioned lexicon. Its corpus statistics always come from the complete 25,555-message export, not the date-filtered subset: `N` is that export size; `df` is the number of normalized, stop-word-free message documents containing a token; `docLength` and `averageDocLength` count those same tokens; and `idf = ln(1 + (N - df + 0.5) / (df + 0.5))`. Score with `sum(idf * tf * (k1 + 1) / (tf + k1 * (1 - b + b * docLength / averageDocLength)))`. Apply each adjustment at most once: add `+2` if any contiguous sequence of at least two normalized, non-stop-word, non-intent query tokens occurs in the normalized message; add `+0.5` when the query has an intent marker and at least three distinct remaining normalized content tokens; and subtract `0.5` when the message has at most two normalized, stop-word-free tokens.
